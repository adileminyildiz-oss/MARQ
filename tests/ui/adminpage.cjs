...
(async()=>{
  ...
  A(r.l.join()==='Vue d’ensemble,Fiche technique,'+MODS.join()&&r.on.join()==='Vue d’ensemble'&&/^RH\d+$/.test(r.rh)&&/Administration/.test(r.nav||''),'client choisi : barre des modules (vue d’ensemble active, alertes par module)',JSON.stringify(r));
